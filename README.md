...
之后正常使用 Claude Code。捕获浏览器默认使用下面的地址；如果端口被占用，启动输出会显示自动选择的新地址：
...
进入某个 session 后，可以查看完整请求时间轴、上下文大小、diff 标识、模型设置、工具数量、模型响应，以及按逻辑层拆开的 request body。
...
捕获文件会保留原始 request body，并在可用时保留响应体。也就是说，里面可能包含 system prompt、用户输入、工具结果、文件片段、命令输出和模型响应。默认只会脱敏常见敏感请求头，不会改写 body 内容。

默认输出目录在 `.claude/prompt-gateway`，本仓库模板会忽略 `.claude`。如果你用 `--output` 指到其他位置，请确认该目录不会被提交到 git，也不要直接分享完整 capture 文件，除非你已经审阅过其中内容。
...
5. 在本地保存捕获到的请求和响应信息。
6. 在 `http://127.0.0.1:8787/` 提供捕获浏览器；如果端口被占用，会自动选择另一个可用端口。
...
| `--port <value>` | `8787` | 本地 gateway 监听端口；被占用时会自动选择可用端口。 |
